...
class JournalApp
{
    ...
    public class JournalEntry
    {
        ...
        public JournalEntry(string prompt, string response, string mood, List<string> tags, string location, string category)
        {
    ...
    public class Journal
    {
        ...
        public void AddNewEntry()
        {
            ...
            Console.Write("Where did you write this journal entry (Location)? ");
            string location = Console.ReadLine();
            Console.Write("What category does this entry belong to (e.g., Personal, Work, Reflection)? ");
            string category = Console.ReadLine();

            JournalEntry entry = new JournalEntry(randomPrompt, response, mood, tags, location, category);
            entries.Add(entry);
        }
        ...
        public void ShowEntries()
        {
            ...
            {
                Console.WriteLine($"Date: {entry.Date} | Time: {entry.Time}");
        ...
        public void SaveEntriesToCsv(string filename)
        {
            using (StreamWriter writer = new StreamWriter(filename))
            {
                foreach (var entry in entries)
                {
                    ...
                    writer.WriteLine($"\"{entry.Date}\",\"{entry.Time}\",\"{entry.Prompt}\",\"{entry.Response}\",\"{entry.Mood}\",\"{tags}\",\"{entry.Location}\",\"{entry.Category}\"");
                }
            }
            ...
            if (File.Exists(filename))
            {
                ...
                foreach (var line in lines)
                {
                    ...
                    if (parts.Length == 8)
                    {
                        ...
                        List<string> tags = new List<string>(parts[5].Trim('"').Split(';'));
                        string location = parts[6].Trim('"');
                        string category = parts[7].Trim('"');

                        entries.Add(new JournalEntry(prompt, response, mood, tags, location, category) { Date = date, Time = time });
                    }
                }
